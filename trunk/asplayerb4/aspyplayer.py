...
class TagReader(object):
	def __init__(self, file_path):
		...
		if self.file_path.endswith(".mp3"):
			fp = EasyID3(file_path)
			try:	
					self.title = fp["title"][0]
			except:
			...
			try:
				self.artist = fp["artist"][0]
			except:
				self.artist = u"Unknown"
			try:
				self.album = fp["album"][0]
			except:
			...
			self.number = -1
				...
				try:
					...
					if len(self.file_path) > 18:
						...
						self.title = UnicodeHelper.safe_unicode(self.file_path)
				try:
					self.artist = fp['\xa9ART'][0]
				except:
					self.artist = u"Unknown"
				try:
					self.album = fp['\xa9alb'][0]
				except:
					self.album = u"Unknown"
				try:
					self.year = fp['\xa9day'][0]
				except:
					self.year = ""
				try:
					self.comment = fp['\xa9cmt'][0]
				except:
					self.comment = ""
				self.number = -1
...
class Music(object):
	...
	def init_music(self):
		if self.file_path:
			id3_reader = TagReader(self.file_path)
...
class LogFactory(object):
	def create_for(name):
		file_path = "E:\\data\\aspyplayer\\log.txt"
...
class FileSystemServices:
	def find_all_files(self, root_dir, file_extension):
		result = []
		predicate = lambda f: ending(f)
		def ending(file):
			if type(file_extension) == type (("", "")):
				bol = False
				for x in file_extension:
					bol = bol or file.endswith(x)
				return bol
			else:
	...
	def get_all_music_files_path_in_device(self):
		all_musics_in_c = self.find_all_files("C:\\", (".mp3", ".m4a"))
		all_musics_in_e = self.find_all_files("E:\\", (".mp3", ".m4a"))
		all_music = []
