...
class ID3(DictProxy, mutagen.Metadata):
    ...
    def load(self, filename, known_frames=None, translate=True):
        ...
        try:
            self.__fileobj = file(filename.encode("utf8"), 'rb')
        except:
            print filename
        self.__filesize = getsize(filename.encode("utf8"))
        try:
            try:
                self.__load_header()
            except EOFError:
...
class StringSpec(Spec):
    ...
    def validate(s, frame, value):
        if value is None: return None
        if isinstance(value, (str,unicode)) and len(value) == s.len: return value
...
class MultiSpec(Spec):
    ...
    def validate(self, frame, value):
        ...
        if self.sep and isinstance(value, (str,unicode)):
...
class TCON(TextFrame):
    ...
    def __set_genres(self, genres):
        if isinstance(genres, (str,unicode)): genres = [genres]
